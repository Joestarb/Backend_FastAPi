...
def predict_mental_health(avg_daily_usage: float = Query(..., description="Horas promedio de uso diario de redes sociales")):
    ...
    try:
        ...
        # Calcular Addicted_Score (0-10)
        # Invertimos el puntaje de salud mental (10 - pred) porque a peor salud mental, mayor adicción
        # Luego promediamos con las horas de uso (normalizadas a escala 0-10)
        # Asumimos que el máximo de horas en el dataset es 10 para normalizar
        max_hours = 10  # Puedes ajustar esto según tu dataset
        normalized_usage = min(avg_daily_usage, max_hours) / max_hours * 10
        addicted_score = (normalized_usage + (10 - pred)) / 2
        ...
        # Interpretación del Addicted_Score
        if addicted_score > 7:
            addiction_level = "Alta adicción"
            addiction_advice = "Se recomienda intervención inmediata y posible ayuda profesional"
        elif 5 <= addicted_score <= 7:
            addiction_level = "Adicción moderada"
            addiction_advice = "Se recomienda establecer límites y monitorear el uso"
        else:
            addiction_level = "Baja adicción"
            addiction_advice = "Uso dentro de parámetros considerados saludables"
    ...
    return JSONResponse({
        ...
        "addicted_score": {
            "puntaje": round(addicted_score, 2),
            "nivel": addiction_level,
            "interpretacion": addiction_advice,
            "calculo": f"Promedio entre horas normalizadas de uso ({normalized_usage:.1f}/10) y salud mental invertida ({(10 - pred):.1f}/10)"
        },
        "interpretacion": {
